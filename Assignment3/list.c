 ...
 * ----------------------
 * @brief  Allows to add a new node to the list respecting an order - descending by count
 *
 * @param list The list where the node will be added (i.e., a pointer to the first element in the list).
 * @param new The node to be added to the list.
 *
...
/**
 * Function:  add_ascending_order
 * ----------------------
 * @brief  Allows to add a new node to the list respecting an order - ascending by count
 *
 * @param list The list where the node will be added (i.e., a pointer to the first element in the list).
 * @param new The node to be added to the list.
 *
 ...
 * Function: is_in
 * --------------
 * @brief  Checks if name is already a word in list
 *
 * @param list The list (i.e., pointer to head node) where the function will be applied.
 * @param name The pointer to check 
 *
 * @ Increments statistic and returns 1 if name is in list
 */
int is_in(node_t *list, char *name) {
    node_t *curr = NULL;
	
	for (curr = list; curr != NULL; curr = curr->next) {
		if (strncmp(curr->word, name, 80)==0){
			curr->count++;
			return 1;
		}
	}
 ...
 * Function: descrease
 * --------------
 * @brief  Decreases statistic by a factor of 10
 *
 * @param list The list (i.e., pointer to head node) where the function will be applied.
 *
 * @ Only used for question 3
 */
